...
/**
 * context-data - Parser and validator for claude.md files
 *
...
class ContextData {
  ...
  _parseFileReferences() {
    ...
    while ((match = fileRefRegex.exec(this.content)) !== null) {
      ...
      // Categorize by filename (case-insensitive)
      if (lowerFilePath.includes('service.claude.md')) {
        this.subFiles.services.push({ path: filePath, absolutePath });
      } else if (lowerFilePath.includes('client.claude.md')) {
        this.subFiles.clients.push({ path: filePath, absolutePath });
      } else if (lowerFilePath.includes('library.claude.md')) {
        this.subFiles.libraries.push({ path: filePath, absolutePath });
      } else if (lowerFilePath.includes('database.claude.md')) {
