...
describe('simple-node-service', () => {
  ...
  const fixturePath = path.join(baseDir, 'simple-node-service');
  const claudeMdPath = path.join(fixturePath, 'claude.md');
  let metadata;

  beforeAll(async () => {
    // Run the plugin to generate claude.md
    // NOTE: For now, this expects claude.md to already exist
    // Future: Implement actual plugin execution via Claude API
